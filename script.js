...
// Formater un prix en format K/M
function formatPriceForMessage(price) {
    if (price >= 1000000) {
...
// Générer le message d'affichage
async function generateDisplayMessage() {
    try {
        // Charger les armes depuis la base de données
        if (!window.db) {
            showNotification('Firestore non disponible', 'error');
            return;
        }

        // Recharger les armes depuis Firestore
        await loadWeaponsFromFirestore();
        
        // Filtrer les armes avec quantité > 0
        const weaponsInStock = weaponsCache.filter(w => w.quantity > 0);
        
        if (weaponsInStock.length === 0) {
            showNotification('Aucune arme en stock', 'error');
            return;
        }
        ...
        // Ajouter le titre avec blanchiment si activé
        if (launderingEnabled) {
            message += `VENTE D'ARMES/BLANCHIMENT ${launderingPercentage}% 🔥\n\n`;
        } else {
            message += `VENTE D'ARMES 🔥\n\n`;
        }

        // Trier les armes par prix de vente (décroissant)
        const sortedWeapons = [...weaponsInStock].sort((a, b) => b.salePrice - a.salePrice);

        // Ajouter chaque arme au message
        sortedWeapons.forEach(weapon => {
            const formattedPrice = formatPriceForMessage(weapon.salePrice);
            message += `🔫 ${weapon.name} — ${formattedPrice} €\n`;
        });

        // Vérifier s'il y a des "tablettes illégales" (on peut chercher par nom ou ajouter une catégorie)
        // Pour l'instant, on ajoute juste les armes. Si vous voulez ajouter d'autres catégories, on peut le faire.
        ...
        showNotification('Erreur lors de la génération du message', 'error');
    }
}
...
// Fonction de débogage pour le bouton Affiche
function showDebugInfo() {
    ...
    const btnGenerate = document.getElementById('btn-generate-message');
    const launderingEnabled = document.getElementById('setting-laundering-enabled');
    const launderingPercentage = document.getElementById('setting-laundering-percentage');
    const messageContainer = document.getElementById('generated-message-container');
    const messageTextarea = document.getElementById('generated-message');
    const btnCopy = document.getElementById('btn-copy-message');
    
    info.push('=== VÉRIFICATION DES ÉLÉMENTS ===');
    info.push(`Bouton Générer: ${btnGenerate ? '✓ Trouvé' : '✗ Manquant'}`);
    info.push(`Case Blanchiment: ${launderingEnabled ? '✓ Trouvé' : '✗ Manquant'}`);
    info.push(`Champ Pourcentage: ${launderingPercentage ? '✓ Trouvé' : '✗ Manquant'}`);
    info.push(`Conteneur Message: ${messageContainer ? '✓ Trouvé' : '✗ Manquant'}`);
    info.push(`Textarea Message: ${messageTextarea ? '✓ Trouvé' : '✗ Manquant'}`);
    info.push(`Bouton Copier: ${btnCopy ? '✓ Trouvé' : '✗ Manquant'}`);
    
    // Vérifier la section dans le HTML
    const messageSection = Array.from(document.querySelectorAll('h4')).find(h => h.textContent.includes('Message d\'affichage'));
    info.push(`\nSection HTML: ${messageSection ? '✓ Trouvée' : '✗ Manquante'}`);
    
    // Vérifier Firestore
    info.push(`\n=== VÉRIFICATION FIRESTORE ===`);
    info.push(`Firestore disponible: ${window.db ? '✓ Oui' : '✗ Non'}`);
    
    // Vérifier les armes en cache
    info.push(`\n=== INVENTAIRE ===`);
    info.push(`Armes en cache: ${weaponsCache ? weaponsCache.length : 0}`);
    if (weaponsCache && weaponsCache.length > 0) {
        const inStock = weaponsCache.filter(w => w.quantity > 0);
        info.push(`Armes en stock: ${inStock.length}`);
        if (inStock.length > 0) {
            info.push(`\nExemples d'armes en stock:`);
            inStock.slice(0, 3).forEach(w => {
                info.push(`- ${w.name}: ${w.quantity} unité(s) - ${formatNumberWithSpaces(w.salePrice)}€`);
            });
        }
    }
    ...
    info.push(`generateDisplayMessage: ${typeof generateDisplayMessage === 'function' ? '✓ Existe' : '✗ Manquante'}`);
    info.push(`formatPriceForMessage: ${typeof formatPriceForMessage === 'function' ? '✓ Existe' : '✗ Manquante'}`);
    
    // Afficher toutes les infos
    const message = info.join('\n');
    alert(message);
    
    // Aussi dans la console si disponible
