...
function createWeaponCard(weapon) {
    ...
    if (weapon.qtyPropre > 0 || weapon.qtySale > 0) {
        const parts = [];
        if (weapon.qtyPropre > 0) parts.push(`✅ ${formatNumberWithSpaces(weapon.qtyPropre)}`);
        if (weapon.qtySale > 0) parts.push(`💀 ${formatNumberWithSpaces(weapon.qtySale)}`);
        stockHint = `<div class="stock-split">${parts.join(' · ')}</div>`;
    }

    card.innerHTML = `
        <div class="weapon-header">
            <div class="weapon-name">${weapon.name}</div>
            <div class="weapon-quantity" title="Stock total">${formatNumberWithSpaces(quantity)}</div>
        </div>
        ${stockHint}
...
function showAdminTab(tabName) {
    ...
    if (tabName === 'gestion') {
        ...
        document.getElementById('admin-tab-prices').classList.add('active');
        document.getElementById('btn-admin-prices-tab').classList.add('active');
    }
}
